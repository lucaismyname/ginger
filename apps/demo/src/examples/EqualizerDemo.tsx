...
function EqualizerPanel() {
  ...
  return (
    <div className="rounded-2xl border border-orange-200 bg-orange-50/70 p-5 shadow-sm ring-1 ring-orange-100">
      <div className="text-xs font-semibold uppercase tracking-wide text-orange-700">Equalizer</div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-5">
        {bands.map((band, index) => (
          <label
            key={band.frequency}
            className="flex flex-col gap-1 rounded-xl border border-orange-200/80 bg-white/80 px-2 py-2"
          >
            ...
            />
            <span className="text-center font-mono text-[10px] text-orange-800">
              {(band.gain ?? 0).toFixed(1)} dB
            </span>
          </label>
        ))}
      </div>
