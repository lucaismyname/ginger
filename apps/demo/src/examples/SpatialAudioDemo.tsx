...
function SpatialPanel() {
  ...
  return (
    <div className="rounded-2xl border border-violet-200 bg-violet-50/70 p-5 shadow-sm ring-1 ring-violet-100">
      <div className="text-xs font-semibold uppercase tracking-wide text-violet-700">Spatial audio</div>
      <p className="mt-2 text-sm leading-relaxed text-violet-900">
        HRTF panner on the active media element (
        <code className="rounded bg-white/80 px-1 font-mono text-[11px]">@lucaismyname/ginger/spatial</code>
      ...
      <div className="mt-4 space-y-3">
        <label className="flex flex-col gap-1 text-xs font-medium text-violet-800">
          ...
          />
          <span className="font-mono text-[11px] text-violet-600">{sourceX.toFixed(2)}</span>
        </label>
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            className="rounded-lg border border-violet-300 bg-white px-3 py-1.5 text-xs text-violet-900 hover:bg-violet-100"
            onClick={() => {
              setSourceX(0);
      ...
      <div className="mt-4 space-y-3 border-t border-violet-200/80 pt-4">
        <div className="flex items-center justify-between font-mono text-xs text-violet-800">
          <Ginger.Current.Elapsed />
          <span className="truncate text-[11px]">{g.currentTrack?.title}</span>
          <Ginger.Current.Duration />
        </div>
        <Ginger.Control.SeekBar className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-violet-200 accent-violet-700" />
        <div className="flex flex-wrap items-center gap-2">
          <Ginger.Control.PlayPause className="rounded-lg bg-violet-700 px-4 py-1.5 text-sm font-medium text-white hover:bg-violet-800" />
          <Ginger.Control.Volume className="h-1.5 w-28 cursor-pointer appearance-none rounded-full bg-violet-200 accent-violet-700" />
        </div>
      </div>
    </div>
  );
