...
function FilePeaksPanel() {
  ...
  return (
    <div className="rounded-2xl border border-sky-200 bg-sky-50/70 p-5 shadow-sm ring-1 ring-sky-100">
      <div className="text-xs font-semibold uppercase tracking-wide text-sky-700">Waveform (file peaks)</div>
      <p className="mt-2 text-sm leading-relaxed text-sky-900">
        ...
        <code className="rounded bg-white/80 px-1 font-mono text-[11px]">@lucaismyname/ginger/waveform</code>
        ). Distinct from the live analyzer waveform demo.
      </p>

      <div className="mt-4 flex h-32 items-end justify-between gap-px rounded-xl border border-sky-200 bg-gradient-to-b from-white to-sky-100/50 px-1 pb-1 pt-3">
        {isLoading && peaks.length === 0 ? (
          <span className="w-full text-center text-xs text-sky-600">Loading peaks…</span>
        ) : error ? (
          ...
          peaks.map((p, i) => (
            <div
              key={i}
              className="min-w-0 flex-1 rounded-t bg-sky-600/90 dark:bg-sky-400/90"
      ...
      <div className="mt-4 space-y-3 border-t border-sky-200/80 pt-4">
